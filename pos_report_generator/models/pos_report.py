    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            if data.get('date_from'):
                query += "Where  l.pos_categ_id = 2 AND (l.date_order >= '%s') " % data.get('date_from').strftime('%Y-%m-%d 00:00:00')
                term = 'AND '
            if data.get('date_to'):
                if not data.get('date_from'):
                    term +=  ' AND '
                query += term + "(l.date_order <= '%s') " % data.get('date_to').strftime('%Y-%m-%d 11:59:59')
